...
let camera3D, scene, renderer, cube, light;
let dir = 0.05;
...
function init3D() {
    ...
    camera3D = new THREE.PerspectiveCamera(69, window.innerWidth / window.innerHeight, 0.1, 1000);
    ...
    var videoGeometry = new THREE.PlaneGeometry(240, 240);
    p5CanvasTexture = new THREE.Texture(myCanvas.elt);  
    ...
    myAvatarObj.position.set(0, 0, 0);


    const geometry = new THREE.BoxGeometry(2, 1, 2, 2, 2, 2);
    const material = new THREE.MeshBasicMaterial({ map: p5CanvasTexture, transparent: false, opacity: 0, side: THREE.DoubleSide });
    ...
    let panotexture = new THREE.TextureLoader().load("yh.jpg");
...
function animate() {
    ...
    if (cube.scale.x > 8 || cube.scale.x < -4) {
        dir = -dir;
    }
